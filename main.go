...
func main() {
	...
	if len(os.Args) < 2 {
		...
		case "init":
			...
			f.Parse(os.Args[2:])
			if len(f.Args()) > 0 {
				err = initc(ctx, cwd, f.Arg(0), opt.recurse, opt.hidden)
			} else {
		...
		case "cp":
			...
			f.Parse(os.Args[2:])
			if len(f.Args()) > 1 {
				err = cp(ctx, cwd, f.Arg(0), f.Arg(1), opt.recurse, opt.hidden)
			} else {
		...
		case "mv":
			...
			f.Parse(os.Args[2:])
			if len(f.Args()) > 1 {
				err = mv(ctx, cwd, f.Arg(0), f.Arg(1), opt.recurse, opt.hidden)
